  ...
  describe 'ユーザー新規登録' do
    ...
    context '新規登録できないとき' do
      ...
      it 'last_name_fgnが空では登録できない' do
        @user.last_name_fgn = ''
        @user.valid?
        expect(@user.errors.full_messages).to include("Last name fgn can't be blank")
      end

      it 'first_name_fgnが空では登録できない' do
        @user.first_name_fgn = ''
        @user.valid?
        expect(@user.errors.full_messages).to include("First name fgn can't be blank")
      end
      ...
      it 'last_name_fgnが全角カナでなければ登録できない' do
        ...
        @user.valid?
        expect(@user.errors.full_messages).to include('Last name fgn is invalid')
      end
      it 'first_name_fgnが全角カナでなければは登録できない' do
        @user.first_name_fgn = 'mail'
        @user.valid?
        expect(@user.errors.full_messages).to include('First name fgn is invalid')
      end
    end
  end
end
